...
class CPPApp:
    def __init__(self, root):
        ...
        self.pause_removal_enabled = tk.BooleanVar(value=True)
        self.vad_check = tk.Checkbutton(
            btn_row, text="Enable voiced-only extraction (VAD)", variable=self.vad_enabled,
            bg=BG_COLOR, font=BTN_FONT, onvalue=True, offvalue=False
        )
        self.vad_check.pack(side=tk.LEFT, padx=(8, 2))
        self.pause_removal_check = tk.Checkbutton(
            btn_row, text="Enable pause removal", variable=self.pause_removal_enabled,
            bg=BG_COLOR, font=BTN_FONT, onvalue=True, offvalue=False
        )
        self.pause_removal_check.pack(side=tk.LEFT, padx=(2, 8))
        # ------------------------------------------------------------
    ...
    def run_analysis(self):
        ...
        try:
            results = extract_cpp(
                self.audio_path, region=region, method=method, file_type=file_type,
                min_f0=min_f0, max_f0=max_f0,
                vad_enabled=self.vad_enabled.get(),
                pause_removal_enabled=self.pause_removal_enabled.get()
            )
    ...
    def batch_process(self):
        ...
        self.root.update()
        try:
            batch_results = batch_extract_cpp(
                folder_path, method=method, file_type=file_type,
                min_f0=min_f0, max_f0=max_f0,
                vad_enabled=self.vad_enabled.get(),
                pause_removal_enabled=self.pause_removal_enabled.get()
            )
    ...
    def export_csv(self):
        ...
        if self.analysis_result and not self.batch_results:
            ...
            with open(save_path, 'w', newline='') as f:
                writer = csv.writer(f)
                writer.writerow([
                    "filename", "file_type", value_column,
    ...
    def show_about(self):
        ...
        text = tk.Text(about_win, wrap="word", font=("Calibri", 11), bg=BG_COLOR, relief="flat", bd=0, height=22,
                       width=65)
